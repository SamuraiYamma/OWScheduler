...
It uses django's AbstractUser which already stores first name, last name, email, username, and password. This allows
us to use django's secure password hashing but still give our users attributes like university, role, battletag, sr,
and team. The team in this model is an optional foreign key that can be null and/or blank. The default string for a
player is their battletag. It also restricts the roles to either damage, tank, or support.
"""
...
"""
The model for TimeSlot allows users to set their availability. The ID avoids the composite key issue from django.
Since they don't allow composite keys, this is the next best option.
"""
...
class TimeSlot(models.Model):
    ...
    dayOfWeek = models.CharField(max_length=8, choices=DAYS_OF_WEEK)  # monday, tuesday, wednesday, etc.
    hour = models.IntegerField(default=0)
    players_available = models.ManyToManyField(Player, related_name="player_availabilities")
...
class Match(models.Model):
    ...
    defenseTeam = models.ForeignKey('Team', on_delete=models.CASCADE, db_column='defense_id',
                                    related_name='defense_id')  # named defense_id
    winner = models.ForeignKey('Team', on_delete=models.CASCADE, db_column='winner_id',
                               related_name='winner_id')  # named winner_id
    loser = models.ForeignKey('Team', on_delete=models.CASCADE, db_column='loser_id',
                              related_name='loser_id')  # named loser_id
